...
function buscarTodos() {
    for (var i = 0; i < dias.length; i++) {
        ruta = 'assets/horarios/' + dias[i] + '.csv';
        leer(ruta);
    }
}
...
daySelect.addEventListener("change", (e) => {
    ...
    if (dia == 'todos') {
        ...
        matriz = [];
        ruta = 'assets/horarios/' + dia.toUpperCase() + '.csv';
        leer(ruta);
    }
});
...
function buscar(texto, arreglo) {
    ...
    for (var i = 2; i < curso.length; i++) {
        if (mate_doc[i].indexOf(texto) != -1 || curso[i].indexOf(texto) != -1 || lugar[i].indexOf(texto) != -1) {
            var urlImagen = 'assets/lugares/' + lugar[i];
            var png = ".PNG' /> ";
            ...
            var pop =
                `<button onclick="mostrar()"
                     ...
                     data-trigger="focus" 
                     title="${  lugar[i] } " 
                     data-content=" ${ imagen } "> ${ lugar[i] }
                </button>`;
            // data-campo
            tabla += `
                <tr>
                    <td data-campo="Día"> ${  array_dia[i].toUpperCase()} </td>
                    <td data-campo="Hora"> ${ hora[i] }</td> 
                    <td data-campo="Materia - Docente">  <i>  ${ mate_doc[i] }</i> </td>
                    <td data-campo="Curso"> ${ curso[i] } </td>
                    <td data-campo="Ubicación"> ${  pop  } </td>
                </tr>`;
